...
namespace UnityEngine.InputSystem.Samples.RebindUI
{
    ...
    public class RebindActionUI : MonoBehaviour
    {
        ...
        private void PerformInteractiveRebind(InputAction action, int bindingIndex, bool allCompositeParts = false)
        {
            ...
            m_RebindOperation = action.PerformInteractiveRebinding(bindingIndex)
                ...
                .OnCancel(
                    operation =>
                    {
                        // Enable the action once the rebind is cancelled.
                        action.Enable();
                ...
                .OnComplete(
                    operation =>
                    {
                        // Enable the action once the rebind is completed.
                        action.Enable();
                        m_RebindOverlay?.SetActive(false);
                        m_RebindStopEvent?.Invoke(this, operation);

                        // Check for duplicate bindings and if found, remove the set binding override.
        ...
        /// <summary>
        /// Check for duplicate rebindings within the same action map.
        /// Also checks for previously set overrides in the current composite action that is being rebound.
        /// </summary>
        /// <param name="action">InputAction we are rebinding.</param>
        /// <param name="bindingIndex">Current index of the control we are rebinding, mostly used for composites.</param>
        /// <param name="allCompositeParts">True if the action is a composite action such as a Vector2.</param>
        /// <returns></returns>
        private bool CheckDuplicateBindings(InputAction action, int bindingIndex, bool allCompositeParts = false)
        {
            ...
            // Check for duplicate bindings within the same composite action. This will only check previously
            // set overrides.
            if (allCompositeParts)
            {
